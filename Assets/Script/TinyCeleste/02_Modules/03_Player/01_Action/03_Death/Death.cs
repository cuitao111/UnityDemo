...
{
    public class Death : EntityComponent
    {
        // 是否处于死亡状态
        public bool isDead;
        ...
        private void Awake()
        {
            ...
            player = GetComponentNotNull<PlayerCharacter>();
        ...
        {
            if (isDead) return;
            //m_Rigidbody2DWrapper.Pause(false);      //将速度置为0
            isDead = true;
            // 隐藏Player
            playerPause.Hide();
            // 产生Death动画预制体（播放完后自动销毁）
            //利用工厂类S_Dust_Factory生成灰尘E_Dust，定时销毁
            //销毁时调用 监听对象的OnDustDestory
        ...
        /*public void OnDeathAnimFinish()
        {
            ...
            m_Rigidbody2DWrapper.Resume();              //恢复暂停前的速度
